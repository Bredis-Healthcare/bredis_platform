...
function AdminThreadsList () {
    ...
    const fetchData = async () => {
        ...
        try {
            const request = await axios.get(`/admin/threads?memberId=${pageInfo.resourceId}`);
                                ...
                                data.list.map((thread, index) => (
                                    <div key={thread.id}>
                                        ...
                                        <div className={`w-[1097px] h-[90px] left-[76px] relative`}>
                                            ...
                                            <div className=" left-0 top-0 absolute text-zinc-500 text-lg font-normal font-['Inter']">{thread.category}</div>
                                            {
                                                thread.unreadCustomerMessages > 0 ? (
                                                    <div className="UnreadNoti">
                                                        ...
                                                        <div className="1 left-[543px] top-[33px] absolute text-zinc-500 text-lg font-normal font-['Inter']">읽지 않은 메시지가 {thread.unreadCustomerMessages}건 있습니다.</div>
                                                    </div>
                                                ) : (<div />)
                                            }
                                            ...
                                            <Link to="/admin/members/threads/detail" state={{resourceId: thread.id}}>
